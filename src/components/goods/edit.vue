      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-position="top" label-width="100px">
        <el-tabs :tab-position="'left'" v-model="activeIndex" :before-leave="beforeTabLevae" @tab-click="tabClicked">
          ...
          <el-tab-pane label="商品图片" name="3">
            ...
            -->
            <el-upload class="upload-demo" :action="uploadURL" :on-preview="handlePreview" :on-remove="handleRemove" list-type="picture" :headers="headersObj" :on-success="handleSuccess" drag accept="image/png,image/jpeg">
...
export default {
  ...
  methods: {
    ...
    //获取所有商品数据
    async getGoodList(){
      this.editId=window.sessionStorage.getItem('edit')
      const { data: res } = await this.$http.get(`goods/${this.editId}`)
      console.log(`goods/${this.editId}`);
      // console.log(res)
      if (res.meta.status !== 200) {
        return this.$message.error('获取商品列表失败!')
      }
      ...
      for (let index = 0; index < addForm.pics.length; index++) {
